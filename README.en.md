...
| **Three-layer anti-hallucination** | ① The server detects and truthfully states whether a message actually contains images — if a user says "attached" but nothing arrived, the model never pretends to see it; ② only acknowledge what was actually provided; ③ **commitments that require a decision are never made on the user's behalf** — reassure and escalate to human review |
