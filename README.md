...
> 开源定位：本项目提供知识库系统的**整体思路与可扩展框架**。它不绑定任何业务——你的资料如何组织、回答长成什么样，取决于你定义的知识与使用方式；本项目只提供机制与骨架。
...
| 知识图谱 | LLM 实体/关系抽取（按库可开关），G6 画布人工校准：编辑/合并/确认/位置记忆/自动分层布局 |
| 混合检索 | 向量 + 图谱融合，结果可溯源；图谱检索按真实关系收紧，支持排除语义（"除了 X"）与全量枚举（"所有数据"），意图由 LLM 解析 |
...
| **三重防幻觉** | ① 系统检测并如实告知"消息中是否有图片附件"——客户说传了但没传时，模型绝不假装看见；② 只承认实际提供的内容；③ 涉及退款/赔偿等**需要决策的承诺一律不替使用者拍板**，引导人工核实 |
| **审计驱动进化** | 每次问答留痕（问题+回答）可一键标注 好/差：好的自动提炼沉淀，生成时自动参考相似好案例（few-shot）；差的导出复盘清单 |
| 只读账号 | 角色化权限：只读账号可查看一切并体验，所有修改入口前端隐藏 + 后端 403 双重拦截 |
| 多租户密钥 | 每端独立密钥绑定知识范围，可过期/吊销，密钥哈希存储，全程审计 |
| 三层提示词 | 可按密钥 / 全局 / 内置配置回答提示词——口吻、语言、风格由你定义，框架只保证"基于知识、干净输出" |
