...
| **三重防幻觉（自我监测·实事求是）** | 生成前先"自我核查"，把事实与想象分开：① **事实层**——消息里是否真的含图片等附件由系统检测并如实告知模型，模型只基于真实存在的内容作答，绝不"脑补"看到/收到；② **规则层**——只承认输入中实际提供的信息，把"声称"与"事实"严格区分；③ **决策层**——需要拍板、承诺的结论（赔付、变更、答复承诺等）不由模型代替使用者决定，一律转人工核实。回答全程可溯源、可审计，出现偏差可被定位并复盘修正 |
